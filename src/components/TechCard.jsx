...
const [mouseEnter, setMouseEnter] = useState(false);
  return (
    <div className={`perspective-[1000px] aspect-1/1 flex flex-col items-center flex-1 justify-center lg:w-auto md:w-auto w-[90%] text-[#182934] lg:mx-0 lg:mt-0 md:mx-0 md:mt-0 mx-2 mt-2 ${mouseEnter ? "animate-bouncy" : ""} rounded-2xl ${highlight ? "lg:scale-130 md:scale-130 scale-110" : ""} ${highlight ? "animate-bouncy" : ""}`} onMouseEnter={() => {setTextShow(true);setMouseEnter(true)}} onMouseLeave={() => {setTextShow(false);setMouseEnter(false)}}>
      <div
        ref={cardRef}
        {...tiltHandlers}
      ...
      >
        ...
        {textShow && <div className='absolute top-0 left-0 w-full h-full lg:flex md:flex hidden justify-center items-center bg-[#315a6d90] rounded-2xl p-2'>
            <span className='lg:block md:block hidden text-center lg:text-lg md:text-lg text-[white]'>{label}</span>
            </div>
        }
        <div
          ref={glowRef}
